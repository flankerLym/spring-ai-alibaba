...
//		}
	...
	private void handleSessionVariables(WorkflowContext context) {
		Map<String, Object> sessionMap = (Map<String, Object>) context.getVariablesMap().get("conversation");
		if (MapUtils.isEmpty(sessionMap)) {
			sessionMap = (Map<String, Object>) context.getVariablesMap().get("session");
		}
		if (MapUtils.isEmpty(sessionMap)) {
			return;
		}
		...
		if (MEMORY_TYPE_SELF.equals(type)) {
			...
			if (param != null) {
				...
				if (shortValue instanceof List) {
					...
					int maxMessages = shortTermMemory.getRound() == null ? shortValues.size()
							: Math.max(shortTermMemory.getRound(), 0) * 2;
					int startIndex = Math.max(0, shortValues.size() - maxMessages);
					for (int i = startIndex; i < shortValues.size(); i++) {
						Object value = shortValues.get(i);
						if (value instanceof Message) {
							resultList.add(withMemoryPrefix((Message) value, shortTermMemory));
						}
						else {
							...
							if (messageMap != null) {
								...
								if (MessageRole.USER.getValue().equals(role)) {
									resultList.add(new UserMessage(prefix(content, shortTermMemory.getUserPrefix())));
								}
								else if (MessageRole.ASSISTANT.getValue().equals(role)) {
									resultList
										.add(new AssistantMessage(prefix(content, shortTermMemory.getAssistantPrefix())));
								}
	...
	private Message withMemoryPrefix(Message message, ShortTermMemory shortTermMemory) {
		if (message instanceof UserMessage) {
			return new UserMessage(prefix(message.getText(), shortTermMemory.getUserPrefix()));
		}
		if (message instanceof AssistantMessage) {
			return new AssistantMessage(prefix(message.getText(), shortTermMemory.getAssistantPrefix()));
		}
		return message;
	}

	private String prefix(String content, String prefix) {
		return StringUtils.isBlank(prefix) ? content : prefix + content;
	}
