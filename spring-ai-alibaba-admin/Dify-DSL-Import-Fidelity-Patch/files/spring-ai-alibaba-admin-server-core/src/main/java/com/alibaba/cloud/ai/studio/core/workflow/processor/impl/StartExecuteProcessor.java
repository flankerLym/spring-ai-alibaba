...
public class StartExecuteProcessor extends AbstractExecuteProcessor {
	...
	public NodeResult innerExecute(DirectedAcyclicGraph<String, Edge> graph, Node node, WorkflowContext context) {
		...
		if (!CollectionUtils.isEmpty(sessionParamList)) {
			Map<String, Object> conversationObj = new HashMap<>();
			sessionParamList.forEach(sessionParam -> {
				...
				conversationObj.put(sessionParam.getKey(), value);
			});
			context.getVariablesMap().put(ParamSourceEnum.conversation.name(), conversationObj);
			// Keep the historical alias readable for workflows saved by older versions.
			context.getVariablesMap().put("session", conversationObj);
		}
