...
const checkLLMNodeDataValid = (data: ILLMNodeData) => {
  ...
  const importedFromDify = !!data.node_param.imported_dify;

  if (!data.node_param.sys_prompt_content && !importedFromDify) {
    errorMsg.push({
      label: $i18n.get({
  ...
  if (!data.node_param.prompt_content && !importedFromDify) {
    errorMsg.push({
      label: $i18n.get({
